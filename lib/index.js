'use strict'
/**
 * dsh-vidgen-plugin · Host 半区(官方插件包 lib/index.js)
 * 由 pkg-6 动态 host 全文(cordis_define code.host)转换而来:
 * - module.exports = { name, inject, apply }
 * - harness.handle/defineTool/registerTool → 本地适配器 + ctx.tools + REST /dsh-vidgen/rpc
 * - SCRIPTS_DIR 相对 __dirname 解析(环境变量 VIDGEN_SCRIPTS_DIR 可覆盖)
 * - ctx.timeout → setTimeout / Promise sleep
 */
const { join } = require('node:path')

const name = 'vidgen'
const inject = ['tools', 'webServer', 'settings', 'shell']

const API_BASE = '/dsh-vidgen'
...
async function apply(ctx) {
    ...
    const IMG_MODELS = ['agnes-image-2.5-flash', 'agnes-image-2.1-flash', 'agnes-image-2.0-flash']
    // ── 脚本目录:请把下面路径改为本仓库 scripts/ 的实际绝对路径(或用环境变量 VIDGEN_SCRIPTS_DIR 覆盖) ──
    const SCRIPTS_DIR = process.env.VIDGEN_SCRIPTS_DIR || join(__dirname, '..', 'scripts')
    ...
    const GRAPH_FILE = SCRIPTS_DIR + '/graphscript.js'
    const SNAP_FILE = SCRIPTS_DIR + '/snapscript.js'

    const history = []
    function upsertHistory(entry) {
      const id = entry.task_id || entry.video_id || entry.url || ('e' + Date.now())
      const idx = history.findIndex((h) => h.task_id === id)
      const row = { kind: entry.kind || 'video', prompt: entry.prompt || '', task_id: id, video_id: entry.video_id || '', status: entry.status || 'pending', url: entry.url || '', seconds: entry.seconds || '', size: entry.size || '', error: entry.error || '', ts: entry.ts || Date.now() }
    ...
    async function loadHistory() {
      try {
        ...
        if (Array.isArray(j.items)) j.items.slice(0, 12).forEach((h) => {
          // 兼容旧记录:图片有 url 即视为已完成(旧版写历史时漏了 status)
          if (h && h.kind === 'image' && h.url && h.status !== 'completed') upsertHistory(Object.assign({}, h, { status: 'completed' }))
          else upsertHistory(h)
        })
      } catch (e) {}
    }

    // ── 重启后恢复:对没有终态的视频任务自动继续轮询,完成即回写 URL ──
    let resumeStarted = false
    async function resumePendingTasks() {
      if (resumeStarted) return
      resumeStarted = true
      try {
        const items = history.filter((h) => h.kind !== 'image' && !h.url && (h.task_id || h.video_id) && h.status !== 'failed' && h.status !== 'completed')
        if (!items.length) return
        let todo = items.slice(0, 6)
        for (let round = 0; round < 60 && todo.length; round++) {
          const still = []
          for (const item of todo) {
            const id = item.video_id || item.task_id
            let t = null
            try { t = await pollVideoTask(id) } catch (e) {}
            const st = t ? (t.status || 'pending') : 'pending'
            if (st === 'completed') {
              const out = completedResult(t, item)
              upsertHistory(Object.assign({}, item, out))
            } else if (st === 'failed') {
              upsertHistory(Object.assign({}, item, { status: 'failed', error: JSON.stringify(t.error || t).slice(0, 300) }))
            } else {
              still.push(item)
              upsertHistory(Object.assign({}, item, { status: st, progress: t && t.progress || 0 }))
            }
          }
          todo = still
          if (todo.length) await new Promise((resolve) => setTimeout(resolve, 20000))
        }
        schedulePersist()
      } catch (e) {}
    }

    // ── 画布状态持久化(关闭/重启后恢复节点与连线;读取带 Promise 缓存,消除挂载竞态) ──
    let graphData = null
    let graphScheduled = false
    ...
    async function ensureGraphLoaded() {
      ...
      if (!graphLoading) graphLoading = (async () => {
        try {
          const binInfo = await resolveNodeBin()
          if (!binInfo.bin) return
          const r = await runNode(binInfo.bin, GRAPH_FILE, 'read', 30000)
          let j = {}
          try { j = JSON.parse(r.stdout || '{}') } catch (e) { j = {} }
          if (j.graph && Array.isArray(j.graph.nodes)) graphData = j.graph
        } catch (e) {}
        graphLoaded = true
      })().finally(() => { graphLoading = null })
      return graphLoading
    }
    function scheduleGraphPersist() {
      if (graphScheduled) return
      graphScheduled = true
      setTimeout(async () => {
        graphScheduled = false
        try {
          const binInfo = await resolveNodeBin()
          if (!binInfo.bin) return
          await runNode(binInfo.bin, GRAPH_FILE, JSON.stringify({ graph: graphData }), 30000)
        } catch (e) {}
      }, 500)
    }
    ...
    async function runNode(bin, scriptPath, argStr, arg2, timeoutMs) {
      const command = bin + ' ' + JSON.stringify(scriptPath) + ' ' + JSON.stringify(argStr || '') + (arg2 ? ' ' + JSON.stringify(arg2) : '')
    ...
    // 通过 stdin 传 payload:消除命令行 ARG_MAX / shell 转义对长 payload / UTF-8 特殊字符的影响
    async function runNodeWithStdin(bin, scriptPath, stdinPayload, timeoutMs) {
      const command = bin + ' ' + JSON.stringify(scriptPath)
      const spec = ctx.shell.resolve({
        command: command,
        timeoutMs: timeoutMs || 120000,
        stdoutMaxBytes: 4 * 1024 * 1024,
    ...
    async function httpJson(method, url, headers, bodyObj, timeoutMs) {
      ...
      // 用 Buffer.from(..., 'utf8').toString('base64') 替代 btoa:btoa 只能编码 Latin-1,中文/全角符号会抛 Invalid character
      ...
      // 改用 stdin 传 payload,避免命令行 ARG_MAX / shell 转义对长 payload 的影响
      const r = await runNodeWithStdin(binInfo.bin, HTTP_FILE, payload, timeoutMs)
    ...
    function buildImageBody(params, defaultModel) {
      ...
      const wantB64 = !!params.return_base64
      if (wantB64) {
        // 按 AGNES 文档:文生图用顶层 return_base64:true;图生图/多图合成用 extra_body.response_format:b64_json
        if (imgs.length) {
          body.extra_body = { response_format: 'b64_json', image: imgs }
        } else {
    ...
    function completedResult(data, base) {
      ...
      const out = {
        task_id: base.task_id, video_id: base.video_id || base.task_id, status: 'completed', progress: 100, url,
    ...
    // ── 官方插件适配:pkg-6 动态 harness → tools / webServer / REST ──
    let defineTool = null
    try {
      const m = await import('@deepseek-ai/dsh-tools')
      defineTool = (m && (m.defineTool || (m.default && m.default.defineTool))) || null
    } catch (e) { /* tools 不可用时仅保留 REST 与设置页 */ }
    const handlers = {}
    const harness = {
      defineTool: (spec) => defineTool ? defineTool(spec) : null,
      registerTool: (ctx, tool) => { if (ctx.tools && tool) ctx.tools.register(tool) },
      handle: (name, fn) => { handlers[name] = fn; return fn },
    }
        ...
        try {
          ...
          if (!(params && params.wait === false)) {
            ...
            for (let i = 0; i < 45; i++) {
              ...
              let t
              try { t = await pollVideoTask(created.video_id || base.task_id) } catch (e) {
    ...
    harness.handle('image-generate', async (args) => {
      try {
        ...
        if (url) finalUrl = url
          ...
          // base64 输出 → 转 data URI,直接喂给前端 <img> (避免落盘)
          finalUrl = 'data:image/png;base64,' + String(b64).replace(/\s+/g, '')
        } else {
          return { error: '生图接口未返回图片(既无 url 也无 b64_json): ' + JSON.stringify(data).slice(0, 300) }
        }
        upsertHistory({ kind: 'image', prompt: String(args.prompt || ''), url: finalUrl.slice(0, 60) + '...', status: 'completed' })
        return { url: finalUrl }
      } catch (e) { return { error: String((e && e.message) || e) } }
    })
    ...
    harness.handle('video-history-remove', async (args) => {
      const id = args && (args.task_id || args.video_id || args.url)
      if (!id) return { ok: false, error: '缺少 id' }
      const idx = history.findIndex((h) => (h.task_id === id) || (h.video_id === id) || (h.url === id))
      if (idx >= 0) { history.splice(idx, 1); schedulePersist() }
      return { ok: true, removed: idx >= 0 }
    })
    harness.handle('video-history-clear', async () => {
      history.length = 0
      schedulePersist()
      return { ok: true }
    })
    // ── 画布快照(list / save / load / remove) ──
    harness.handle('vidgen-snapshot-list', async () => {
      try {
        const binInfo = await resolveNodeBin()
        if (!binInfo.bin) return { items: [], error: '找不到 node 二进制' }
        const r = await runNode(binInfo.bin, SNAP_FILE, 'list', 30000)
        let j = {}
        try { j = JSON.parse(r.stdout || '{}') } catch (e) { j = {} }
        return { items: Array.isArray(j.items) ? j.items : [] }
      } catch (e) { return { items: [], error: String((e && e.message) || e) } }
    })
    harness.handle('vidgen-snapshot-save', async (args) => {
      try {
        const binInfo = await resolveNodeBin()
        if (!binInfo.bin) return { ok: false, error: '找不到可用的 node 二进制' }
        await ensureGraphLoaded()
        const payload = JSON.stringify({ name: (args && args.name) || '', graph: graphData || { nodes: [], edges: [] } })
        // 命令行传 payload(JSON 字符串,中文/特殊字符已正确转义;若未来大画布撞 ARG_MAX 可改 stdin)
        const r = await runNode(binInfo.bin, SNAP_FILE, 'save', payload, 30000)
        let j = {}
        try { j = JSON.parse(r.stdout || '{}') } catch (e) { j = { error: r.stdout || r.stderr } }
        return j.ok ? { ok: true, key: j.key, item: j.item || null } : { ok: false, error: String(j.error || '保存失败') }
      } catch (e) { return { ok: false, error: String((e && e.message) || e) } }
    })
    harness.handle('vidgen-snapshot-load', async (args) => {
      try {
        const binInfo = await resolveNodeBin()
        if (!binInfo.bin) return { graph: null, error: '找不到 node 二进制' }
        const r = await runNode(binInfo.bin, SNAP_FILE, 'read', (args && args.key) || '', 30000)
        let j = {}
        try { j = JSON.parse(r.stdout || '{}') } catch (e) { j = { graph: null, error: r.stdout || r.stderr } }
        return { graph: j.graph || null, error: j.error || '' }
      } catch (e) { return { graph: null, error: String((e && e.message) || e) } }
    })
    harness.handle('vidgen-snapshot-remove', async (args) => {
      try {
        const binInfo = await resolveNodeBin()
        if (!binInfo.bin) return { ok: false, error: '找不到 node 二进制' }
        const r = await runNode(binInfo.bin, SNAP_FILE, 'remove', (args && args.key) || '', 30000)
        let j = {}
        try { j = JSON.parse(r.stdout || '{}') } catch (e) { j = { error: r.stdout || r.stderr } }
        return { ok: !!j.ok, removed: !!j.removed }
      } catch (e) { return { ok: false, error: String((e && e.message) || e) } }
    })
    harness.handle('video-config-save', async (args) => {
      try {
        const binInfo = await resolveNodeBin()
        if (!binInfo.bin) return { ok: false, error: '找不到可用的 node 二进制' }
        const prov = Object.assign({}, (args && args.provider) || {})
        // 系统兜底场景:用户未保存过 user 文件,但当前 active(apiUrl/name/model 已就绪)是系统兜底
        // 只在「没填 apiKey 且 apiUrl/name 与当前 active 完全一致」(用户仅改 model 或同名覆盖)时
        // 才把当前 active 的 apiKey 兜底过来;否则保持原校验(防止误把系统 Key 灌到新平台配置里)
        if (!String(prov.apiKey || '').trim()) {
          const cfg = await readConfig()
          const a = cfg.video && cfg.video.active
          if (a && a.apiKey && String(prov.apiUrl || '').trim() === String(a.apiUrl || '').trim() && String(prov.name || '').trim() === String(a.name || '').trim()) {
            prov.apiKey = a.apiKey
          }
        }
    ...
    harness.handle('image-config-save', async (args) => {
      try {
        ...
        if (!String(prov.apiKey || '').trim()) {
          const cfg = await readConfig()
          const a = cfg.image && cfg.image.active
          if (a && a.apiKey && String(prov.apiUrl || '').trim() === String(a.apiUrl || '').trim() && String(prov.name || '').trim() === String(a.name || '').trim()) {
            prov.apiKey = a.apiKey
          }
        }
    ...
    loadHistory().then(() => resumePendingTasks())

    // ── REST RPC:浏览器 client 通过 /dsh-vidgen/rpc 调用原 harness.handle 全部方法 ──
    ctx.effect(() => ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        const handler = handlers[body && body.name]
        if (!handler) { sendJson(res, 404, { error: 'unknown rpc: ' + (body && body.name) }); return }
        try {
          const result = await handler((body && body.args) || {})
