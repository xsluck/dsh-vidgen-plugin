...
.vidgen-foot-wrap{flex:0 0 100% !important;width:100% !important;box-sizing:border-box;min-width:0}`
    ...
    const IMG_SIZES = ['4K', '3K', '2K', '1K', '1280x720', '1024x1024', '1024x768']
    const IMG_MODELS = ['agnes-image-2.1-flash', 'agnes-image-2.0-flash']
    const IMG_REF_KEYS = ['ref1', 'ref2', 'ref3', 'ref4']
    const KF_FRAME_COUNT = 3
    const DEFAULT_DATA = {
      image: { url: '' },
      text2image: { prompt: '', size: '2K', ratio: '16:9', model: '', returnBase64: false, outUrl: '' },
      image2image: { prompt: '', size: '2K', ratio: '16:9', model: '', ref1: '', ref2: '', ref3: '', ref4: '', returnBase64: false, outUrl: '' },
    ...
    function NodeStudio() {
      ...
      const buildImageBody = (node) => {
        const b = { model: node.data.model || (imgCfg && imgCfg.model) || 'agnes-image-2.5-flash', prompt: String(node.data.prompt || '') }
        if (!String(node.data.prompt || '').trim()) return { error: '请填写生图/编辑提示词' }
        const sz = node.data.size || '2K'
        if (sz === '2K' || sz === '1K') {
          b.size = sz
          // 图生图默认不传 ratio —— 让 AGNES 按输入图比例输出,避免把原图强行裁/拉到 16:9 破坏构图
          if (node.data.ratio && node.type !== 'image2image') b.ratio = String(node.data.ratio)
        } else {
          b.size = sz
        }
        // 收集参考图:image2image 必填;text2image 可选(支持「多图合成」工作流)
        const imgs = []
        if (node.type === 'image2image' || node.type === 'text2image') {
          for (const p of IMG_REF_KEYS) {
            const u = findRefUrl(node, p)
            if (u) imgs.push(u)
          }
          if (node.type === 'image2image' && !imgs.length) return { error: '图生图/多图合成至少需要 1 张参考图(连线、URL 或上传本地图)' }
        }
        // 返回格式:按 AGNES 文档严格区分 —— 文生图用顶层 return_base64:true;图生图用 extra_body.response_format:b64_json
        const wantB64 = !!node.data.returnBase64
        if (wantB64) {
          if (imgs.length) {
            // 图生图或多图合成:用 extra_body.response_format: "b64_json"
            b.extra_body = { response_format: 'b64_json' }
            if (imgs.length) b.extra_body.image = imgs
          } else {
            // 文生图:顶层 return_base64: true
            b.return_base64 = true
            b.extra_body = { response_format: 'url' } // 占位,host 端会忽略,因为 return_base64=true
          }
        } else {
      ...
      const ensureAncestors = async (nid) => {
        ...
        const collect = (id) => {
          ...
          if (!node) return
          // 只收集上游祖先节点,不收 nid 本身 —— 否则 nid 会在 ensureAncestors
          // 的第 0 层 batch 里跑一次,handleRunNode 之后又跑一次 → 一次点击触发两次生图
          if (id !== nid) {
            edges.forEach((ed) => { if (ed.to.nid === id) collect(ed.from.nid) })
            if (isImgGen(node)) need.push(node)
          } else {
            // nid 自身:仍继续向上收祖先,但 nid 不入 need
            edges.forEach((ed) => { if (ed.to.nid === id) collect(ed.from.nid) })
          }
        }
        collect(nid)
      ...
      const renderNode = (n) => {
        ...
        if (n.type === 'image') {
          ...
          // 多图合成也支持 text2image:可选参考图折叠(只有填了参考图才进入多图合成模式)
          if (!is2i) {
            const hasAnyRef = IMG_REF_KEYS.some((p) => String(n.data[p] || '').trim() || edges.find((ed) => ed.to.nid === n.id && ed.to.name === p))
            const showRefs = !!adv[n.id + ':refs']
            body.push(el('div', { className: 'vg-nfld' }, [el('button', { style: { background: 'transparent', border: 'none', color: 'inherit', cursor: 'pointer', fontSize: 11, opacity: .6, textAlign: 'left', padding: 0 }, onClick: () => setAdv((a) => Object.assign({}, a, { [n.id + ':refs']: !showRefs })) }, showRefs ? '▾ 收起多图合成' : '▸ 多图合成(留空则为文生图,1 张 = 图生图,2+ 张 = 多图合成)')]))
            if (showRefs || hasAnyRef) {
              IMG_REF_KEYS.forEach((p, i) => body.push(refRow(n, p, '参考' + (i + 1), true)))
            }
          }
          body.push(el('div', { style: { display: 'flex', gap: 6 } }, [
            ...
            el('div', { className: 'vg-nfld', style: { flex: 1 } }, [el('label', null, is2i ? '宽高比(图生图默认按输入图比例)' : '宽高比'), el('select', { value: n.data.ratio || '16:9', onChange: (e) => setNodeData(n.id, 'ratio', e.target.value) }, (cfg && cfg.ratios || ['16:9', '9:16', '1:1', '4:3', '3:4']).map((v) => el('option', { key: v, value: v }, v)))]),
          ]))
          const showAdv = !!adv[n.id]
          body.push(el('div', { className: 'vg-nfld' }, [el('button', { style: { background: 'transparent', border: 'none', color: 'inherit', cursor: 'pointer', fontSize: 11, opacity: .6, textAlign: 'left', padding: 0 }, onClick: () => setAdv((a) => Object.assign({}, a, { [n.id]: !showAdv })) }, showAdv ? '▾ 收起高级参数' : '▸ 高级参数(模型/返回格式)')]))
          if (showAdv) {
            body.push(el('div', { className: 'vg-nfld' }, [el('label', null, '模型'), el('select', { value: n.data.model || '', onChange: (e) => setNodeData(n.id, 'model', e.target.value) }, [el('option', { key: '', value: '' }, '默认 ' + ((imgCfg && imgCfg.model) || 'agnes-image-2.5-flash'))].concat(IMG_MODELS.concat((imgCfg && imgCfg.model) ? [imgCfg.model] : []).filter((m, i, a) => a.indexOf(m) === i).map((m) => el('option', { key: m, value: m }, m))))]))
            body.push(el('div', { className: 'vg-nfld' }, [el('label', null, '返回格式'), el('select', { value: n.data.returnBase64 ? 'b64' : 'url', onChange: (e) => setNodeData(n.id, 'returnBase64', e.target.value === 'b64') }, [
              el('option', { key: 'url', value: 'url' }, 'URL(默认,服务端返回 HTTPS 图片链接)'),
              el('option', { key: 'b64', value: 'b64' }, 'Base64(直接以 data:image/png;base64 形式返回 — 文生图用顶层 return_base64:true;图生图/多图合成用 extra_body.response_format:"b64_json")'),
            ])]))
          }
    ...
    function VideoConfig() {
      ...
      // busy / msg / err 按 kind 拆分,确保视频/图片两个 pCard 互不影响
      // (之前共享一个 state 导致点视频保存 → 整个页面按钮都被 disable,看起来"任何点击都在保存")
      const [vBusy, setVBusy] = React.useState('')
      const [iBusy, setIBusy] = React.useState('')
      const [vMsg, setVMsg] = React.useState('')
      const [iMsg, setIMsg] = React.useState('')
      const [vErr, setVErr] = React.useState('')
      const [iErr, setIErr] = React.useState('')
      const busyOf = (kind) => kind === 'video' ? vBusy : iBusy
      const setBusyOf = (kind) => kind === 'video' ? setVBusy : setIBusy
      const msgOf = (kind) => kind === 'video' ? vMsg : iMsg
      const setMsgOf = (kind) => kind === 'video' ? setVMsg : setIMsg
      const errOf = (kind) => kind === 'video' ? vErr : iErr
      const setErrOf = (kind) => kind === 'video' ? setVErr : setIErr
      const complain = (kind) => (e) => setErrOf(kind)('❌ ' + String((e && e.message) || e))
      ...
      const onSave = (kind) => {
        ...
        const sB = setBusyOf(kind), sM = setMsgOf(kind), sE = setErrOf(kind)
        if (!String(e.name).trim() && !String(e.apiUrl).trim() && !String(e.apiKey).trim()) { sE((kind === 'video' ? '视频' : '图片') + ':请先填写名称与 API 信息'); return }
        sB('save'); sE(''); sM('')
        rpc(kind === 'video' ? 'video-config-save' : 'image-config-save', { provider: e }).then((r) => {
          sB('')
          if (r && r.ok) { sM((kind === 'video' ? '✅ 视频' : '✅ 图片') + '提供者已保存并设为当前 ⭐'); refresh(true) }
          else sE('❌ ' + String((r && r.error) || '保存失败'))
        }).catch(complain(kind))
      }
      const onSetCurrent = (kind, id) => {
        ...
        rpc(kind === 'video' ? 'video-config-set' : 'image-config-set', { providerId: id }).then((r) => {
          sB('')
          if (r && r.ok) { sM((kind === 'video' ? '✅ 视频' : '✅ 图片') + '当前提供者已切换'); refresh(true) }
          else sE('❌ ' + String((r && r.error) || '切换失败'))
        }).catch(complain(kind))
      }
      const onRemove = (kind, id) => {
        ...
        const sB = setBusyOf(kind), sM = setMsgOf(kind), sE = setErrOf(kind)
        if (p && p.sys) { sE((kind === 'video' ? '视频' : '图片') + ':系统兜底提供者不可删除(保存自己的提供者后即可管理)'); return }
        sB('rm'); sE(''); sM('')
        rpc(kind === 'video' ? 'video-config-remove' : 'image-config-remove', { providerId: id }).then((r) => {
          sB('')
          if (r && r.ok) { sM((kind === 'video' ? '✅ 视频' : '✅ 图片') + '提供者已删除'); refresh(false) }
          else sE('❌ ' + String((r && r.error) || '删除失败'))
        }).catch(complain(kind))
      }
      const onTest = (kind) => {
        const sM = setMsgOf(kind), sE = setErrOf(kind)
        sM(''); sE('')
        rpc(kind === 'video' ? 'video-models' : 'image-models', {}).then((r) => {
          if (r && r.error) sE('❌ ' + String(r.error))
          else if (kind === 'video' && r && Array.isArray(r.videoModels)) sM('✅ 当前视频提供者连接成功。「🔄 获取模型」可拉取模型列表')
          else if (kind === 'image' && r && Array.isArray(r.models)) sM('✅ 当前图片提供者连接成功。「🔄 获取模型」可拉取模型列表')
        }).catch(complain(kind))
      }
      const onFetchModels = (kind) => {
        const e = kind === 'video' ? vEdit : iEdit
        const sB = setBusyOf(kind), sM = setMsgOf(kind), sE = setErrOf(kind)
        sB('models'); sE(''); sM('⏳ 正在请求 ' + ((e.apiUrl && e.apiUrl.trim()) || '当前提供者') + ' /models …')
        rpc(kind === 'video' ? 'video-models-by' : 'image-models-by', { apiUrl: e.apiUrl, apiKey: e.apiKey }).then((r) => {
          ...
          if (r && r.error) { sE('❌ ' + String(r.error)) }
            ...
            const cur = (e.model || '').trim()
            const needPick = !cur || !list.includes(cur)
            if (list.length && needPick) {
              const next = { ...e, model: list[0] }
              if (kind === 'video') setVEdit(next); else setIEdit(next)
              sM('✅ 获取到 ' + list.length + ' 个模型(共 ' + (r && r.count || 0) + ' 个,已过滤)。已把默认模型同步为: ' + list[0] + '(点击「💾 保存并设为当前」即落盘)')
            } else if (list.length) {
              sM('✅ 获取到 ' + list.length + ' 个模型(共 ' + (r && r.count || 0) + ' 个,已过滤)。当前默认模型「' + cur + '」仍在列表中,无需更改')
            } else {
              sM('⚠️ 服务端返回了 ' + (r && r.count || 0) + ' 个模型,但都不符合视频/图片过滤规则,未填入下拉。请手动填写默认模型名')
            }
          }
        }).catch(complain(kind))
      }
      const onPickModel = (kind, v) => {
        if (v) {
          ...
          setMsgOf(kind)('已填入模型: ' + v)
        }
      }
      const onClearAll = async () => {
        const ok = await askConfirm('清除全部用户提供者', '⚠️ 将删除你保存的所有视频/图片提供者(名称、API 地址、Key),且不可恢复。\n\n继续后将回退到系统兜底配置。', '全部清除')
        if (!ok) return
        // 清除影响两边 → 清空两边的消息与 busy
        setVBusy(''); setIBusy(''); setVMsg(''); setIMsg(''); setVErr(''); setIErr('')
        rpc('video-config-clear', {}).then((r) => {
          if (r && r.ok) { setVMsg('已清除全部用户提供者,回退系统兜底'); setVModels([]); setIModels([]); refresh(false) }
          else setVErr('❌ ' + String((r && r.error) || '清除失败'))
        }).catch((e) => setVErr('❌ ' + String((e && e.message) || e)))
      }
        ...
        return el('div', { style: { border: '1px solid var(--border-1,rgba(128,128,128,.25))', borderRadius: 10, padding: '10px 12px', marginBottom: 10 } }, [
          ...
          el('div', { className: 'vg-row' }, [
            ...
            el('div', { className: 'vg-field', style: { flex: 1 } }, [
              el('label', null, '&nbsp;'),
              el('div', { className: 'vg-row' }, [
                el('button', { className: 'vg-sec-btn', disabled: busyOf(kind) === 'set' || !(edit.id && !isActiveEdit), title: edit.id && isActiveEdit ? '已是当前提供者' : '把当前编辑的提供者切换为当前使用', onClick: () => onSetCurrent(kind, edit.id) }, '⭐ 设为当前'),
                el('button', { className: 'vg-sec-btn danger', disabled: busyOf(kind) === 'rm' || !edit.id, title: '删除当前编辑的提供者', onClick: () => onRemove(kind, edit.id) }, '🗑 删除'),
              ]),
            ]),
          ]),
          el('div', { style: { borderTop: '1px dashed var(--border-1,rgba(128,128,128,.25))', margin: '8px 0', paddingTop: 8 } }, [
            ...
            el('div', { style: { display: 'flex', gap: 6, alignItems: 'flex-start' } }, [
              ...
              el('button', { className: 'vg-sec-btn', style: { marginTop: 20, flex: 'none' }, disabled: busyOf(kind) === 'models', onClick: () => onFetchModels(kind) }, busyOf(kind) === 'models' ? '⏳ 获取中…' : '🔄 获取模型'),
            ]),
            el('div', { className: 'vg-btnrow' }, [
              el('button', { className: 'vg-btn', disabled: busyOf(kind) === 'save', onClick: () => onSave(kind) }, busyOf(kind) === 'save' ? '保存中…' : '💾 保存并设为当前'),
              ...
              el('button', { className: 'vg-sec-btn', disabled: busyOf(kind) === 'test', onClick: () => onTest(kind) }, '🔌 测试当前'),
            ]),
          ]),
        ])
      }
      return [
        ...
        // 视频/图片各自的 msg / err 分行显示(互不污染)
        (vMsg || vErr) ? el('div', { className: 'vg-cfgline', style: { marginTop: 8 } }, [
          vMsg ? el('div', { style: { color: '#2ea043' } }, vMsg) : null,
          vErr ? el('div', { className: 'vg-error' }, vErr) : null,
        ]) : null,
        (iMsg || iErr) ? el('div', { className: 'vg-cfgline', style: { marginTop: 4 } }, [
          iMsg ? el('div', { style: { color: '#2ea043' } }, iMsg) : null,
          iErr ? el('div', { className: 'vg-error' }, iErr) : null,
        ]) : null,
