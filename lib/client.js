      ...
      const onFetchModels = (kind) => {
        ...
        rpc(kind === 'video' ? 'video-models-by' : 'image-models-by', { apiUrl: e.apiUrl, apiKey: e.apiKey }).then((r) => {
          ...
          if (r && r.error) { setErr('❌ ' + String(r.error)) }
            ...
            // 自动把当前默认模型同步到列表第一个(若当前 model 为空 / 在新列表里查不到 → 用第一个)
            // 这样获取模型后,"默认模型"输入框立刻反映,工作区(新建节点 / agent 工具)即可使用新模型
            ...
            if (list.length && needPick) {
              ...
              setMsg('✅ 获取到 ' + list.length + ' 个模型(共 ' + (r && r.count || 0) + ' 个,已过滤)。已把默认模型同步为: ' + list[0] + '(点击「💾 保存并设为当前」即落盘)')
            } else if (list.length) {
              setMsg('✅ 获取到 ' + list.length + ' 个模型(共 ' + (r && r.count || 0) + ' 个,已过滤)。当前默认模型「' + cur + '」仍在列表中,无需更改')
            } else {
              setMsg('⚠️ 服务端返回了 ' + (r && r.count || 0) + ' 个模型,但都不符合视频/图片过滤规则,未填入下拉。请手动填写默认模型名')
            }
          }
        }).catch(complain)
      }
